function moduleProject3() {
  ...
  function buildFooter(footerData) {
    ...
    companyInfoDiv.classList.add('company-ifo')
    ...
    contactEmailP.innerHTML = `Email: <a href="mailto:${footerDate.contactEmail}"> ${footerData.contactEmail}</a>`
    ...
    for (let platform in footerDate.socialMedia) {
      let socialMediaLink = document.createElement('a')
      socialMediaLink.href = footer.Date.socialMedia[platform]
      socialMediaLink.textContent = platform.charAt(0).toUpperCase() + platform.slice(1)   
    }

    let currentYear = new Date().getFullYear()
    let copyright = document.createElement('div')
    copyright.textContent = `(c) ${footerDate.companyName.toUpperCase()} ${currentYear}`
